...
function clickHandle() {
  ...
  useEffect(() => {
    gsap.timeline().fromTo(".linksComputer", {
        opacity:0,
        y:-100
    },
    {
    ...
    <>
        <div className="hidden md:block">
          <div className="linksComputer z-50 fixed top-0 bottom-0 left-0 right-0 bg-white rounded-b-xs shadow-2xl w-full h-fit flex flex-row justify-between items-center px-[1rem] py-[0.5rem] border-b border-slate-300">
                <Link className='linksComputer cursor-pointer transition-all duration-300 ease-in rounded-md hover:text-red-500 mx-[1rem] px-6 py-3 list-none' to='home' spy={true} smooth={true} offset={-100} duration={500}>MaorOvadya</Link>
              <ul className='flex cursor-pointer'>
                    <Link className='linksComputer transition-all duration-500 ease-in rounded-md hover:text-white hover:bg-red-500 mx-[1rem] px-6 py-3' to="about" spy={true} smooth={true} offset={-100} duration={200}>About</Link>
                    <Link className='linksComputer transition-all duration-500 ease-in rounded-md hover:text-white hover:bg-red-500 mx-[1rem] px-6 py-3' to='projects' spy={true} smooth={true} offset={-100} duration={200}>Projects</Link>
                    <Link className='linksComputer transition-all duration-500 ease-in rounded-md hover:text-white hover:bg-red-500 hover:border-white mx-[1rem] px-6 py-3' to='contact' spy={true} smooth={true} offset={-100} duration={200}>Contact</Link>
                    <Link className='linksComputer transition-all duration-500 ease-in rounded-md text-red-500  border border-red-500 hover:text-white hover:border-white hover:bg-red-500 mx-[1rem] px-6 py-3' to='resume'>Resume</Link>
              </ul>
          </div>
        </div>
        ...
        <div className="md:hidden block linksMobile cursor-pointer z-50 realtive top-0 bottom-0 left-0 right-0 bg-white rounded-b-xs shadow-2xl w-full h-fit flex-row justify-between items-center py-[0.5rem] border-b border-slate-300">
            {open ? 
              <li className='linksMobile cursor-pointer transition-all duration-300 ease-in hover:text-red-500 mx-[1rem] p-6 list-none border-b border-slate-300 w-[50%]'>
                <Link to='home' spy={true} smooth={true} offset={-100} duration={500}>MaorOvadya</Link>
              </li>
            :
            ...
            <div className={`z-50 bg-white w-full h-72 pb-3 rounded-b-xs shadow-2xl transition-all duration-700 ease-in-out ${open ? "absolute left-0 " : "absolute left-[-1490px] "}`}> 
              {open ?
              <div> 
